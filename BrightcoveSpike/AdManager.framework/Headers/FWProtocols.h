...
/**
 *	Set video display base UIView. Video ads are rendered within the base view in the same frame.
...
/**
 *	<b>Deprecated</b>
 *	Please use [FWContext setVideoDisplayBase:uiView] to set a UIView instance for non-fullscreen video player to display video ads, or use [ FWContext setMoviePlayerFullscreen:YES] if content video is played in fullscreen mode.
 *	When setMoviePlayerController is invoked, AdManager will automatically set the movie player's view as the video display base, which means [FWContext setMoviePlayerController:player] is equivalent in effect as [FWContext setVideoDisplayBase:player.view].
 */
 ...
 *		- FW_SLOT_OPTION_INITIAL_AD_FIRST_COMPANION_OR_NO_STAND_ALONE: Ask ad server to fill this slot with the first companion ad, but nerver deliver a stand alone ad if there is no companion ad
 ...
 *  \param	baseUnit	The base units that the renderer can support, separated by comma.
